...
**Revision:** 7 — §4.9 corrects §4.3: neither the health bar nor the plate can be usefully resized on this client. Sizing removed entirely in 1.0.0; this feature ships colour only.

---

## 1. Scope

Aggro-based colouring of hostile and neutral nameplates from a DPS
perspective.
...
| 4.3 bar resizable | ~~**Available**~~ **Withheld** | the probe was wrong — see §4.9 |
...
**The default bar height is 10.4**, and the shipped `barHeight = 6` was read as
"a genuine reduction rather than a guess that happened to be smaller". That
reasoning was sound about the number and wrong about the world: the write was
never applied. See §4.9.
...
### 4.9 Correction: the health bar is not resizable, and §4.3 said it was

Recorded here because this doc is the source the later phases read, and it
asserted a capability the client does not have.

**The probe measured the call, not the effect.** It read the bar's current height
and wrote that same value back:

```
current: Length = healthBar.GetHeight()
accepted: boolean = pcall(healthBar.SetHeight, healthBar, current)
record("barResizable", accepted ? AVAILABLE : WITHHELD)
```

A write of the current value cannot distinguish an applied write from an ignored
one. Both leave the same number behind. The client accepts `SetHeight` on the
nameplate health bar and then discards it: the bar carries two vertical anchors,
so the next layout pass recomputes the height from them and an explicit size
never survives. §4.3 therefore reported `Available`, §7.2 shipped a resize, and
the resize never once took effect in live play. The `barHeight = 6` note above —
"a genuine reduction rather than a guess" — was reasoning about a number that was
never applied to anything.

**This is the fourth instance of the same error in this project**, and the only
one that reached a shipped feature: writing a value back to itself (here),
returning a bare name and calling it a title (§4.4), printing "no sizing API" one
line before printing the sizing API, and reporting "accepted in combat" on a run
the client blocked. The probe now writes a *different* value, reads it back, and
restores the original — so `barResizable` reports `Withheld`, which is the truth.

**The mechanism that does work is `C_NamePlate.SetNamePlateSize`.** It is
first-party, sanctioned, and resizes the whole plate rather than the bar. Three
properties that matter:

- **Client-wide, not per plate.** One call governs every nameplate. This makes
  Phase 6's geometry path *simpler* than what it replaces: no per-plate write, no
  drift comparison, and no competing-writer race for size. The race remains for
  colour, where Blizzard genuinely does write (§9.5).
- **Combat-protected.** Blocked in combat, accepted out of it. Out-of-combat
  writes persist into the following fight, so the common case needs no deferral;
  a change made *during* a fight arms a retry on `PLAYER_REGEN_ENABLED`, owned by
  the feature so `disable` clears it.
- **Restorable.** The client's own size is recorded once, before the first write,
  on the same record-once discipline as `StyleLedger` and `CVarSteward`.

The "skinnier bars" claim in §1 is retired, and so is the replacement.
`SetNamePlateSize` was shipped on the strength of a read-back: `GetNamePlateSize`
returns the value we wrote, and the nameplate on screen is unchanged. It sizes the
plate's anchor region; the client's nameplate driver keeps laying out the visible
bar from its own inputs.

**Both attempts share one shape: the addon could verify its own write and could not
verify the effect.** Confirming the call was insufficient, and confirming the
stored value was also insufficient. The only sufficient check was a person looking
at a nameplate, and that check failed for both mechanisms.

Nameplate sizing is therefore **removed**, not deferred — the keys, the schema
entries and the two settings controls are gone as of 1.0.0. A control that stores a
number nothing reads is worse than an absent feature: it reports success. What this
feature ships is colour, which is verifiable by looking at it.
...
- ~~Blizzard writes the bar colour but not the bar dimensions on its
  selected-target path.~~ Moot: there is no dimension write of ours to contest
  (§4.9). Plate size is a client-wide setting with one writer.
- ~~Widget geometry remains unrestricted for the health bar, **confirmed in live
  play**: resizing and recolouring both work.~~ **False, and "confirmed in live
  play" was never confirmed** — only the recolour was. Resizing the bar is
  accepted and discarded (§4.9). The restriction is not merely per-region but
  per-operation *within* a region: recolour lands, resize does not.
